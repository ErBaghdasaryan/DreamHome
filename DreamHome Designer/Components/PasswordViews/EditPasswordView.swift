...
final class EditPasswordView: UIView {
    ...
    private func setupUI() {
        ...
        editButton.layer.cornerRadius = 19
        editButton.titleLabel?.font = UIFont(name: "SFProText-Semibold", size: 17)
        editButton.setTitleColor(.white, for: .normal)
        editButton.backgroundColor = UIColor(hex: "#FF2300")
        ...
        self.buttonsStack = UIStackView(arrangedSubviews: [canceleButton, editButton],
                                        axis: .horizontal,
                                        spacing: 8)
...
extension EditPasswordView {
    ...
    static func createAndShow(in viewController: UIViewController, password: String, delegate: @escaping (_ model: String) -> Void, cancelDelegate: (() -> Void)? = nil) -> EditPasswordView {
        let popup = EditPasswordView(frame: viewController.view.bounds)
        popup.delegate = delegate
        popup.cancelDelegate = cancelDelegate
        popup.superViewController = viewController
        popup.passwordField.customTextField.text = password
        popup.alpha = 0.0
